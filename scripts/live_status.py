...
class LiveStatusApp:
    ...
    def _init_redis_connections(self):
        ...
        try:
            self.redis_panda = EigsepRedis(
                host=self.panda_host, port=self.port
            )
            print(
                f"✓ Connected to redis_panda at {self.panda_host}:{self.port}"
            )
        ...
        if self.redis_panda:
            try:
                ...
                status["panda_data"] = {
                    ...
                    "active_streams": list(
                        self.redis_panda.data_streams.keys()
                    ),
                }
            except Exception as e:
        ...
        if self.redis_snap:
            try:
                ...
                status["snap_data"] = {
                    ...
                    "active_streams": list(
                        self.redis_snap.data_streams.keys()
                    ),
                }
            except Exception as e:
        ...
        if self.redis_panda:
            try:
                ...
                health["components"]["panda"] = {
                    "status": (
                        "healthy"
                        if client_alive and metadata_count > 0
                        else "degraded"
                    ),
        ...
        if self.redis_snap:
            try:
                ...
                health["components"]["snap"] = {
                    "status": (
                        "healthy"
                        if config_present and metadata_count >= 0
                        else "degraded"
                    ),
    ...
    def run(self, host="localhost", port=5000, debug=True):
        ...
        print(f"SNAP: {self.snap_host}:{self.port}")
        print()
...
def main():
    ...
    app.run(host=args.web_host, port=args.web_port, debug=not args.no_debug)
