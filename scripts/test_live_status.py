...
This script tests the live status app with dummy data when Redis is not available.
"""
...
def test_app_with_dummy_data():
    ...
    with app.app.test_client() as client:
        ...
        print(f"✓ Health API: Overall status = {health_data['overall_status']}")
        
        # Test sensors endpoint
        response = client.get('/api/sensors')
        assert response.status_code == 200
        sensor_data = json.loads(response.data)
        print(f"✓ Sensors API: {len(sensor_data['sensors'])} sensor categories")
        
        # Test correlator endpoint
        response = client.get('/api/correlator')
        assert response.status_code == 200
        corr_data = json.loads(response.data)
        print(f"✓ Correlator API: {len(corr_data['correlator_data'])} correlator metrics")
    ...
    print("   python live_status.py --panda-host 10.10.10.12 --snap-host 10.10.10.10")
    print("\n💡 To run with local testing (will show connection errors but serve dummy data):")
    print("   python live_status.py --panda-host localhost --snap-host localhost")
