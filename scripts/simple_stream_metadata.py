    ...
    with h5py.File(filepath, "w") as f:
        ...
        for key in all_keys:
            ...
            if key.endswith("_ts"):
                ...
                f.create_dataset(
                    key, data=np.array(numeric_values, dtype=np.float64)
                )
            elif all(isinstance(v, dict) or v is None for v in values):
                # Nested dictionaries - save as JSON strings
                json_strings = [
                    json.dumps(v) if v is not None else "" for v in values
                ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Stream and save metadata from Panda"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--host", type=str, default="10.10.10.11", help="Redis host address"
    )
    parser.add_argument(
        "--autosave-interval",
        type=int,
    ...
    def save_data(reason=""):
        ...
        if len(data_buffer) > 0 and len(data_buffer) > last_saved_count:
            ...
            if reason:
                print(
                    f"Saving data ({reason})... Collected {len(data_buffer)} records"
                )
            else:
    ...
    try:
        while True:
            ...
            if (
                ...
                and len(data_buffer) % args.autosave_interval == 0
            ):
