...
- **MotorClient** (`motor_client.py`) / **MotorZeroer** (`motor_zeroer.py`) - Client-side motor orchestrators used by `scripts/motor_control.py` and `scripts/motor_manual.py`. Both wrap a `picohost.proxy.PicoProxy("motor", ...)` and a `MetadataSnapshotReader`, drive the scan loop and interactive zeroing from outside the `PicoManager` process, and poll Redis metadata for move completion (so a long move never stalls the manager's shared `cmd_loop`).
