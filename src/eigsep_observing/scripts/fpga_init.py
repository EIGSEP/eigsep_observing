"""SNAP supervisor entry point.

Without flags, attaches to a SNAP that is already running and synced
(``sync_time`` is rehydrated from the corr header). Use ``--reinit`` for
a fresh observing block (ADC + FPGA regs + sync); add ``-p``/``-P`` to
also (re)program the bitstream.

Installed as the ``eigsep-fpga-init`` console script and run by the
``eigsep-observe.service`` systemd unit (see ``deploy/systemd/``).
"""
...
def main() -> None:
    ...
    if args.reinit:
        ...
        # Attach path: SNAP is already running; recover sync_time from
        # the header so CorrWriter.add doesn't drop every integration.
    ...
    if args.interactive:
        # Lazy-import so the supervisor service (eigsep-observe.service)
        # never pulls IPython in — it's only needed for -i.
        import IPython
